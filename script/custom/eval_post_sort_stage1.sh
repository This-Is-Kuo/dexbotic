...
stage_dir="${STAGE1_OUTPUT_DIR:-${POST_SORT_FINETUNE_ROOT}/post_sort_stage1_mix_fullft}"
checkpoint="${STAGE1_CKPT:-$(post_sort_latest_checkpoint "${stage_dir}")}"
if [[ -z "${checkpoint}" || ! -d "${checkpoint}" ]]; then
  echo "[ERROR] Stage 1 checkpoint not found. Set STAGE1_CKPT or check ${stage_dir}."
  exit 1
fi
...
eval_root="${POST_SORT_EVAL_ROOT}/post_sort_stage1_mix"
...
echo "[OK] Stage 1 openloop eval complete: ${eval_root}"
